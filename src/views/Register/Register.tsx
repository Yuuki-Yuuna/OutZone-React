...
const Register: React.FC = () => {
  ...
  const toNext = () => {
    setLoading(true)
    form
      .validateFields(['username', 'mailAddress', 'verificationCode'])
      .then((values) => {
        setLoading(false)
        setIstransfrom(true)
      })
  ...
  const sendCapcha = () => {
    form
      .validateFields(['username', 'mailAddress'])
      .then((values: { username: string; mailAddress: string }) => {
        const { username, mailAddress } = values
        setCountDown(60) //用useEffect监测
        getRegisterCode({ username, mailAddress })
          .then((res) => {
  ...
  return (
    <div className='register'>
      <div className='register-box'>
        <div className='glass'>
          <p className='info' style={{ marginTop: '50px' }}>
            Welcome
          </p>
                ...
                <div className='left'>
                  ...
                  >
                    <div className='capcha-box'>
                      ...
                      {countDown ? (
                        <span className='capcha-send'>
                          重新发送<i>{countDown}</i>
                        </span>
                      ) : (
                        <span
                          className='capcha-send'
                          style={{ color: '#096dd9' }}
                          onClick={sendCapcha}
                        >
                          发送验证码
                  ...
                  <Button className='next' type='primary' shape='round' onClick={toNext}>
                    下一步
                  </Button>
                </div>
                <div className='right'>
                  ...
                  <Button className='next' type='primary' shape='round' htmlType='submit'>
                    注册
                  </Button>
                </div>
              </div>
